...
public class Num implements Comparable<Num> {
	...
	public static Num evaluateInfix(String[] expr) {
		...
		String n1 = null; // first Number
		String n2 = null; // second Number
		String opt = null; // operator
		int i = 0;
		
		// Set<String > uniqueOperators=new HashSet<String>();
		// HashMap to store <operator, priority> pairs
		Map<String, Integer> uniqueOperators = new HashMap<String, Integer>();
		...
		while (i < expr.length) {
			...
			if (!uniqueOperators.containsKey(expr[i])) 
				operand.push(expr[i]);
			...
			// When it's a closing parenthesis pop(or evaluate) until we reach opening parenthesis
				...
				// When opening parenthesis, just do nothing and push("(")
				else if (expr[i].equals("(")) 
					operator.push(expr[i]);
				
				// When the priority  of the new operator is more than operator.peek -> push(operator)
				else if (uniqueOperators.get(expr[i]) < uniqueOperators.get(operator.peek())) 
					operator.push(expr[i]);
